package com.hyz.douyin.interaction.job.one;
...
@Component
@Slf4j
public class CreateMongoCollection implements CommandLineRunner {
    ...
    public boolean createCollection(String collectionName) {
        // 设置集合名称
//        String collectionName = "tb_video_favorite";
        // 创建集合并返回集合信息
...
//        return mongoTemplate.collectionExists(collectionName) ? "创建视图成功" : "创建视图失败";
    ...
    public void run(String... args) {
        String favoriteName = CollectionConstant.FAVORITE_COLLECTION_NAME;
        String commentName = CollectionConstant.COMMENT_COLLECTION_NAME;
        if (!mongoTemplate.collectionExists(favoriteName)) {
            createCollection(favoriteName);
            log.info("mongo 集合初始化，集合名称：【" + favoriteName + "】");
        } else {
            log.info("mongo 集合初始化失败，集合【" + favoriteName + "】已存在");
        }
        if (!mongoTemplate.collectionExists(commentName)) {
            createCollection(commentName);
            log.info("mongo 集合初始化，集合名称：【" + commentName + "】");
        } else {
            log.info("mongo 集合初始化失败，集合【" + commentName + "】已存在");
        }
    }
}
